...
def analyze_email(email_record):
    """
    Master pipeline function for PhishGuard.

    Takes one EmailRecord from the database and runs it through all
    six analysis services in order, then saves the result to AnalysisReport.

    Args:
        email_record (EmailRecord): A single email object from the database.

    Returns:
        AnalysisReport: The saved analysis result object.
    """

    logger.info("Starting analysis for EmailRecord ID: %s", email_record.id)


    #  Step 1: Parse the raw email
    # Converts the EmailRecord into a clean structured dictionary.
    # All downstream services consume this parsed_data dict.
    try:
        # Parse structured fields
        parsed_data = parse_email(email_record)
        logger.debug("Step 1 complete: email parsed.")
    except Exception as e:
        logger.error("Step 1 FAILED (parse_email): %s", e)
        parsed_data = {}

    #  Step 2: Header Authentication Analysis
    # Checks SPF, DKIM, DMARC, Reply-To mismatch, Return-Path mismatch.
    # Returns auth_score (0–30).
    try:
    ...
    # Extracts URLs, IPs, domains, and file hashes from the email.
    # Saves each as an IOC row in the database (linked to this email_record).
    try:
        ioc_list = extract_iocs(parsed_data)
        save_iocs(email_record, ioc_list)
        logger.debug("Step 3 complete: IOCs extracted and saved.")
    except Exception as e:
...
# Step 6: ML Classification
    ...
    # Return the saved AnalysisReport object so the caller (view/task/signal)
    # can use the result immediately without re-querying the database.
    return report
